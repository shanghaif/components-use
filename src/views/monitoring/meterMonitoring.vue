            ...
            <div class="detail animated fadeIn" v-if="form.terminal_type=='2'">
                <div class="operation" v-show="operation.tq">
                    <div class="summarize">
                        <el-card shadow="hover">
                            ...
                            <ul class="con">
                                <li>台区总数：<span><strong>{{operation.tq}}</strong>个</span></li>
                                <li>集中器总数：<span><strong>{{operation.vcaddr}}</strong>台</span></li>
                                <li>电能表总数：<span><strong>{{operation.addr}}</strong>台</span></li>
                                <li>单相电能表总数：<span><strong>{{operation.single_meter}}</strong>台</span></li>
                                <li>三相电能表总数：<span><strong>{{operation.triple_meter}}</strong>台</span></li>
                                <li>当日冻结电量：<span><strong>{{operation.count_day}}</strong>KWh</span></li>
                                <li>上日用电总量：<span><strong>{{operation.day}}</strong>KWh</span></li>
                                <li>当月冻结电量：<span><strong>{{operation.count_month}}</strong>KWh</span></li>
                                <li>当上月用电总量：<span><strong>{{operation.month}}</strong>KWh</span></li>
                            </ul>
                        </el-card>
                    </div>
                ...
                <ul class="table">
                    <li v-for="item of electricity" :key="item.id" :class="{offLine_border:!item.status}">
                        <div class="img">
                            <svg-icon icon-class="electricEnergyMeter4" v-show="item.sblx=='1'"></svg-icon>
                            <svg-icon icon-class="electricEnergyMeter5" v-show="item.sblx=='2'"></svg-icon>
                        </div>
                        <div class="info">
                            <p>{{item.sblx | type}}:<span :class="[item.status?'normal':'offLine']">({{item.addr}})</span></p>
                            <p>pn值:{{item.pn}}<span :class="[item.status?'normal':'offLine']">{{item.status | Online}}</span></p>
                            <div>
                                <p>当日冻结电量：<span style="line-height:25px" :class="[item.status?'normal':'offLine']">{{item.freeze_day}}KWh</span></p>
                                <p>当月冻结电量：<span style="line-height:25px" :class="[item.status?'normal':'offLine']">{{item.freeze_month}}KWh</span></p>
                            </div>
                        </div>
                    </li>
                </ul>
            ...
            <div class="detail animated fadeInUp" v-if="form.terminal_type=='1'">
                ...
                <ul class="table">
                    <li class="con" v-for="item of vcon" :key="item.id">
                        ...
                        <div class="info">
                            <p>{{item.name}} <strong :class="[item.online ? 'normal' : 'offLine']">({{item.addr}})</strong> <span :class="[item.online ? 'normal' : 'offLine']">{{item.online | Online}}</span></p>
                            <p>所属台区：{{item.tq}}</p>
                            <p>终端通信地址：{{item.host}}</p>
                            <p :class="[item.masterstation ? 'normal' : 'offLine']">主站连接状态：{{item.masterstation | Master}}</p>
                            <p style="color:#4B8BF4;">本地时钟：{{Time}}</p>
                            <div>
                                <p>在线电表：{{item.count_meter}}/{{item.count_online_meter}}</p>
                                <p class="mr-30">当日抄表情况:{{item.count_meter}}/{{item.count_online_meter}}</p>
...
import { getMeterStatistic,getElectricity,getVcon,QueryMeter,QueryVcon } from "@/api/meterMonitoring"
    ...
    methods:{
        query(){//查询
            ...
            if (this.form.terminal_type=="1"){
                this.Vpager.page=1;
                QueryVcon(this.form.terminal_addr,this.form.isOnline,time,this.Vpager.rows,this.Vpager.rows*(this.Vpager.page-1)).then(res=>{
                    // console.log(res);
                    this.vcon=[];
                ...
                this.Mpager.page=1;
                QueryMeter(this.form.terminal_addr,this.form.isOnline,time,this.Mpager.rows,this.Mpager.rows*(this.Mpager.page-1),Mtype).then(res=>{
                    // console.log(res);
                    this.electricity=[];
        ...
        // 获取统计数据
        getData(objId,time){
            getMeterStatistic(objId,time).then(res=>{
                // console.log(res);
                if (res.status_statistic){
                    ...
                    setTimeout(()=>{
                        if (this.operation.tq){
                            ...
                            this.value3=[];
                            this.value4=[];
                            this.setVlaue(this.data1,this.value1,Stat.all_tq,Stat.all_tq-Stat.online_tq);
                            this.setVlaue(this.data2,this.value2,all_meter,all_meter-Stat.online_tq);
                            this.setVlaue(this.data3,this.value3,Stat.success_meter,all_meter);
                            this.setVlaue(this.data4,this.value4,Stat.intime_meter,all_meter-Stat.intime_meter);
                            this.pie_data();
                        }
                    },1);
                }
        ...
        getElectricityData(objId){
            ...
            getElectricity(objId,this.time,this.Mpager.rows,(this.Mpager.page-1)*this.Mpager.rows,metertype).then(res=>{
                // console.log(res);
        ...
        getVconData(objId){
            this.clearTimer();
            getVcon(objId,this.time,this.Vpager.rows,(this.Vpager.page-1)*this.Vpager.rows).then(res=>{
                console.log(res);
                if (res.count==0){
                    this.noData=false;
                }else {

                ...
                <el-form :model="setForm">
                    ...
                    <div v-show="updateShow" style="display:flex;flex-wrap: wrap;justify-content: space-around;">
                        ...
                        <el-form-item label="远程控制密码" :label-width="formLabelWidth">
                            <el-input show-password v-model="setForm.dbmm" autocomplete="off" placeholder="请输入远程控制密码"></el-input>
                        </el-form-item>
                        <el-form-item label="采集器编号" :label-width="formLabelWidth">
    ...
    methods:{
        ...
        */
        ...
        handleClickVerify(){
            ...
            this.OIData=[];
            // 恢复颜色
            let arr=document.querySelectorAll(".table .el-table__body-wrapper .el-table__row .el-tooltip");
                    ...
                    let time=setInterval(()=>{
                        ...
                        if (this.state=="校验失败"){
                            ...
                            query.find().then(res=>{
                                // console.log(res);
                                this.OIData.push(
                                    {num:this.num,oper: "数据上传",errRow: "",errCell:"",OI:`上传${this.sum}条`,handlers:this.user,time:new Date().toLocaleString()}
                                )
                                for (let i=0;i<res.length;i++){
                                    ...
                                    if (str != "{}"){
                                        ...
                                        for (let key in err){
                                            ...
                                            for (let j=0;j<this.headArr.length;j++){
                                                ...
                                                if (this.headArr[j].en==key || this.headArr[j].en==key+"_web"){
                                                    cell=j;
                                                    //异常颜色
                                                    // console.log(i);
                                            ...
                                            this.OIData.push(
                                                {num:this.num,oper: "数据校验",errRow: `第${i+1}行`,errCell:`${i+1}:${cell+1}`,OI:`${key}:${err[key]}`,handlers:this.user,time:new Date().toLocaleString()}
                                            )
                                        }
                                    }
                                    if (i == res.length-1){
                                        this.num++;
                                        this.OIData.push(
                                            {num:this.num,oper: "数据验证(汇总)",errRow: "",errCell:"",OI:`校验完成${this.sum}条,${this.abnormal}条存在异常`,handlers:this.user,time:new Date().toLocaleString()}
                                        )
                                    }
                                }
                            })
        ...
        handleChangeUnusual(){
            if (this.form.iserr){
                // 左边内容
                let con_arr=document.querySelectorAll(".table .el-table__body-wrapper .el-table__row .el-tooltip");
                // 右边删除按钮
                let btn_arr=document.querySelectorAll(".table .el-table__fixed-right .el-table__row");
                console.log(btn_arr);
                let count=0;
                for (let i=0;i<con_arr.length;i++){
                    if (con_arr[i].getAttribute("style") == "width: 79px; color: rgb(96, 98, 102);"){
                        count++;
                        // 连续41次都是正常颜色，那么就隐藏他
                        if (count == 41){
                            // 总数除以count获取按钮的位置

  ...
  <div class="enrollment">
    <div class="enrollment_header">
      ...
      <div class="alldelete" style="padding:2px;background-color:#dbf0ff;font-size:14px;">
          <p><i class="el-icon-info" style="color:#409eff;margin-right:5px"></i>已选择<span style="margin:0 10px;color:#409eff">{{meterselectdata.length}}</span>项 <el-link type="primary" @click="deleteall" :underline="false">清空</el-link></p>
      </div>
      <div class="table_enrollment">
        ...
        >
          ...
          <el-table-column label="授权状态" align="center">
            <template slot-scope="scope">
                 <el-tag
                        v-if="scope.row.attributes.auth==0"
                        type="warning"
                        effect="dark">
                       未授权
                    </el-tag>
                     <el-tag
                    v-else-if="scope.row.attributes.auth==1"
                    type="success"
                    effect="dark">
                       已授权
                    </el-tag>
          ...
          <el-table-column label="注册状态" align="center">
              <template slot-scope="scope">
                  <el-tag
                        v-if="scope.row.attributes.reg==0"
                        type="warning"
                        effect="dark">
                       未注册
                    </el-tag>
                <el-tag
                    v-else-if="scope.row.attributes.reg==1"
                    type="success"
                    effect="dark">
                       已注册
                    </el-tag>
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
                <el-link type="primary" :disabled="scope.row.attributes.reg==1" @click="regMeter(scope.row,1)" :underline="false" icon="el-icon-edit" style="border-right:1px solid #cccccc;padding-right:5px">注册</el-link>
              <!-- <el-button type="primary" size="small"  >注册</el-button> -->
              <!-- <el-button type="primary" size="small" @click="regMeter(scope.row,0)">查看</el-button> -->
              <el-link type="primary"  @click="regMeter(scope.row,0)" icon="el-icon-view" :underline="false">查看</el-link>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  methods: {
    ...
    deleteMeter(){
        this.$confirm('是否删除已选中未授权电表', '提示', {
         ...
         Promise.all([this.meterIds.map(item=>this.getMeterId(item))]).then(data => {
            this.$message({
              message: "删除成功",
              type: "success"
             });
    ...
    authQr(){
        this.$confirm('是否注册已选中电表', '提示', {
          ...
          AuthMeter(this.meterselectdata,1).then(resultes=>{
              if(resultes){
                 this.$message({
                    type: 'success',
                    message: '已成功注册'

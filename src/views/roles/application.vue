<template>
  <div class="application">
            ...
            <el-col :span="16">
               ...
               <div class="grid-content bg-purple">
                  <dl>
                     ...
                     <dd v-else>{{item.attributes.secret.substr(0,4)+'********************'+item.attributes.secret.substr(24)}}<el-button @click="xianshi(item.id)" size="samll" style="margin-left:10px" round>完整密钥</el-button></dd>
    ...
    <!--新建弹框-->
    <el-dialog title="添加新应用" :visible.sync="dialogVisible" width="40%" :before-close="handleClose">
      <div class="block">
        <el-form ref="form" :model="form" label-width="120px">
          ...
          <el-form-item label="应用名称">
            <el-input v-model="form.name"  style="width:80%" placheholder="请输入应用名称"></el-input>
          </el-form-item>
    ...
    <!--修改应用信息-->
    <el-dialog title="修改应用信息" :visible.sync="update" width="40%" :before-close="handleClose">
      <div class="block">
        <el-form ref="form" :model="form1" label-width="120px">
          <el-form-item label="应用名称">
            <el-input v-model="form1.name"  style="width:80%"></el-input>
          </el-form-item>
...
//   },
  ...
  methods: {
     Define(){
        ...
        Addapp(Number(this.form.time),this.form.name,this.form.product,session).then(resultes=>{
            this.$message({
            type: "success",
            message: "应用创建成功"
         })
         ...
         this.appdata.map(item=>{
            this.selectapp.map(object=>{
               if(item.attributes.product.id==object.id){
                  item.productname = object.attributes.title
                  item.isshow=false
               }
            })
         })
    ...
    handleCurrentChange(val) {
     ...
     updateapp(id){
        this.objectid = id
         ...
         app.get(id).then(res=>{
            ...
            this.form1.name = res.attributes.config.name
         },error=>{
            console.log(error)
         })
     },
     updatedDefine(){
         ...
         app.get(this.objectid).then((object)=>{
            object.set('config',{name:this.form1.name,expires:this.form1.time})

    ...
    <el-dialog title="服务部署" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="客户名称" prop="username">
          ...
          <el-input v-model="ruleForm.version" placeholder="请输入平台版本"></el-input>
          <!-- <el-select v-model="" placeholder="请选择应用版本">
            <el-option label="区域一" value="shanghai"></el-option>
            <el-option label="区域二" value="beijing"></el-option>
          </el-select>-->
  ...
  methods: {
    ...
    addserver(row) {
      ...
      if(row.attributes.product){
        ...
        this.ruleForm.version = row.attributes.product.shuwa_iot_software
      }
      if (
    ...
    updatedLicense(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          product.shuwa_iot_software = this.ruleForm.version
          var License = Parse.Object.extend("License");
          var license = new License();
